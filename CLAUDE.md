...
**Node.js requirement**: >= 22.11. **Ruby 3.3.9 via rbenv** (pinned in `.ruby-version`; `Gemfile.lock` was generated with Bundler 2.4.22 — `gem install bundler:2.4.22 && bundle install`). CocoaPods required for iOS (`bundle exec pod install`). **Tras cada `npm install`, `pod install` ANTES de compilar iOS**: `react-native-quick-crypto` no publica su OpenSSL estático en el tarball — el podspec lo descarga en `pod install` dentro de `node_modules`, así que un `npm install` se lo lleva y el build muere al final con `rsync error: …/QuickCryptoOpenSSL.xcframework/ios-arm64/*: No such file or directory` (los miles de warnings de deprecación que salen antes son ruido). `scripts/run-ios-device.sh` ya lo comprueba y repara solo. `npm run version:sync` reads **`app.json`** (`version` + `versionCode`) and writes them into `package.json` and `ios/QvaPay.xcodeproj/project.pbxproj`; Android reads `app.json` directly via Gradle — all version bumps go through `app.json`. The version also feeds the React Query persister `buster`, so bumping it invalidates the whole persisted query cache on update.
...
`['home', …]` feed del Home (transactions/quickpay/blog/watchlist/promo/announcement/profile) | `['transactions','list',filters]` histórico infinito (los filtros VAN EN LA CLAVE: filtrar = cambiar de query) | `['savings', …]` resumen + movimientos | `['crypto', …]` dashboard del tab Crypto + stocks/históricos | `['coins', kind|history]` catálogo de monedas e históricos | `['store', …]` catálogos Zendit + compras | `['market', …]` Seller Shops | `['assisted', …]` Personal Shopper | `['p2p', …]` monedas/medias/snapshot/perfiles | `['topup', …]` recargas IAP | `['wallet', …]` wallet self-custody (registry persistido; balances/history on-chain con noPersist) | `['energy', …]` precios/órdenes del alquiler de energía TRON | `['user','referrals']` | `['contacts']`

**Queries compartidas entre pantallas** (misma clave = una petición y una caché): `['home','quickpay']` (fila de pago rápido del Home + carrusel de Send — vive en `hooks/useQuickPayQuery.js` para no arrastrar el feed entero a los tests de Send), `['savings','summary']` (`hooks/useSavingsSummaryQuery.js`: BalanceCard + Crypto + Savings — un depósito/retiro invalida `['savings']` y las tres superficies se actualizan), `['coins', kind]` (`hooks/useCoins.js`, reescrito sobre RQ: Add/Withdraw/P2PCreate/PaymentMethods + picker del P2P), `['crypto','coins']` (dashboard + CoinDetail), `['store','topup-countries']` (portada de tienda + PhoneTopupIndex).
...
AppNavigator (Stack)
  ...
  MainStack (authenticated) → Bottom Tabs: Home | Crypto | Keypad | P2P | Store
...
- Base URL from `/config.ts`: dev `http://10.0.0.239:3000/api`, prod `https://api.qvapay.com`. **Dev fallback (`api/apiHost.ts`)**: la primera petición sondea `/status` de la LAN; si no hay respuesta en 15s (`API_DEV_PROBE_TIMEOUT`), el cliente ENTERO pasa a producción el resto de la sesión (cualquier respuesta HTTP, incluso 5xx, mantiene la LAN). Prod nunca sondea. El interceptor de request fija `baseURL` por petición vía `ensureApiHost()`; SSE y PDF leen `getApiBaseUrl()` en call time — nunca copiar `config.API_BASE_URL` a nivel de módulo.
...
- Response interceptor: **401** clears the keychain token (403/422 pass through untouched — 403 is "no permission", not auth); 500 returns `"Ha ocurrido un error, contacte a soporte"`; network errors return `"No se ha podido conectar con el servidor"`. All in Spanish.
...
- `stocksApi.js`: stocks/watchlist data for the Crypto tab
...
- `energyApi.ts`: alquiler de energía TRON (`/v2/energy/prices|quote|rent|orders`), cobrado del saldo QvaPay. `rent` exige `idempotency_key` y devuelve `balance` en el sobre; los códigos de error viajan en `details.code`
...
- `/api/`: 15 modules + `client.js` + la capa de queries: `unwrap.js` (contrato→excepción + política de reintentos), `queryClient.js` (cliente + persister + políticas de persistencia), `queryUtils.js` (`trimToFirstPage`). Las queries por dominio viven junto a sus pantallas: `screens/home/homeQueries.js`, `screens/transaction/transactionsQueries.js` + `sendQueries.js`, `screens/crypto/cryptoQueries.js`, `screens/store/storeQueries.js`, `screens/store/market/marketQueries.js`, `screens/store/assisted/assistedQueries.js`, `screens/p2p/p2pQueries.js`
...
- `/wallet/`: wallet self-custody (en construcción, branch crypto — plan en `~/.claude/plans/`). `registry/`: `types.ts` + `bundled.json` (fallback empaquetado; NINGUNA URL de RPC vive fuera de él), `useRegistry` (RQ `['wallet','registry']`, TTL 1h, fuente = repo público `qvapay/rpc-registry` vía GitHub raw — el espejo rpc.qvapay.com está APLAZADO y fuera del array hasta que exista; el remoto solo gana si su `version` >= bundled), `customRpcs.ts` (PURO: nodos del usuario — ajuste `crypto.customRpcs` en SettingsContext, mapa chainKey→urls https; `applyCustomRpcs` los funde delante con `owner: 'user'` y priority -1, por debajo del 0 de qvapay), `rpcRouter.ts` (PURO, cero react-native: prioridad → salud → latencia, circuit breaker 3 fallos/2min, rota solo en errores de infraestructura, deps inyectadas para tests node; exporta `probeRpc` para validar un nodo antes de guardarlo) y `appRpcRouter.ts` (singleton + salud persistida en `@qpwallet:rpc-health` + **`useEffectiveRegistry`** = remoto/bundled + custom, lo que consumen router y UI — nunca `useRegistry` a pelo desde pantallas — + `useAppRpcRouter`, que monta `WalletProvider` para que el singleton siga el registro efectivo toda la sesión). Regla dura: en `wallet/` solo `keystore.ts` y `appRpcRouter.ts` pueden tocar react-native/AsyncStorage; todo lo demás se testea con `@jest-environment node`. `wallet/chains/` (PURO: saldos EVM/TRON/BTC/Stacks/Solana por el router, bigint exacto) + `wallet/{tron,evm,btc,stacks,solana}/tx.ts` (PURO: construir → VERIFICAR re-parseando → firmar → difundir; QUSD = SIP-010 en Stacks con post-condición deny, identificador del contrato SOLO en el registry). **Solana sin SDK**: `wallet/solana/codec.ts` es un codec propio (mensaje legacy, PDA de ATA, instrucciones) validado contra `@solana/web3.js` con vectores dorados — no añadir `@solana/web3.js` a la app (Hermes pide polyfills); derivación SLIP-0010 ed25519 en `derive.ts` (`deriveEd25519`, `m/44'/501'/0'/0'`). El blockhash caduca en ~60 s: `summary.expiresAt` hace que la confirmación re-prepare + `wallet/assets.ts` (PURO: activo = moneda en una red, lista base, visibilidad/orden/valor USD); queries en `screens/crypto/wallet/walletQueries.ts` y home de la wallet en el tab Crypto (`WalletHome` → `WalletAsset` / `WalletReceive` / `WalletManageAssets`); historial vía `GET /wallet/history` de qpweb (proxy Etherscan/TronGrid/Esplora). Pantalla: Ajustes → Wallet → Mi wallet → Nodos (`subpanels/RpcNodes.tsx` + `RpcNodeAddModal.tsx`): salud/latencia por nodo y alta/baja de nodos custom (se prueban contra el endpoint antes de guardar, máx 5 por cadena)

  **Energía y ancho de banda TRON** (2026-09-20): en TRON una transferencia TRC-20 se paga con energía (~65.000, ~131.000 si el destino nunca recibió ese token); sin ella se queman 13-28 TRX. `getTronResources` devuelve disponible Y límite de cada recurso, y `computeTronBurnBreakdown` desglosa el quemado en energía / ancho de banda / alta de cuenta — desglosado porque **alquilar energía NO paga el ancho de banda**: una cuenta sin banda libre y sin TRX sigue sin poder enviar, y ofrecerle ahí un alquiler es cobrarle por humo. `wallet/tron/energy.ts` (PURO) decide con `shouldOfferRental` → `unblocks | cheaper | no(why)`. La compra va contra `/v2/energy` de qpweb (ver `api/energyApi.ts`), se cobra del **saldo QvaPay** y la energía se delega a la dirección que **FIRMA**, nunca a la que recibe los fondos. Piezas: `energyRentMachine.ts` (PURO: reducer + política de rotación de la clave de idempotencia, que se ata a `dirección|volumen|duración` porque el backend la namespacea por usuario y endpoint, NO por payload — reusarla con otros parámetros devuelve `200 duplicate` con la orden vieja), `useEnergyRent` (efectos, polling del 202 con tope de 120 s, `@qpenergy:pending` en disco para no comprar dos veces tras un cierre de app), `EnergyRentModal` (la MISMA pieza en las dos superficies), pantallas `WalletEnergy` / `WalletEnergyOrders` (entrada desde `WalletAsset` de cualquier activo TRON) y el aviso con CTA de `WalletSendConfirm`, que abre el modal **sin navegar** (la tx TRON caduca en ~60 s) y luego entra en la fase `waitingEnergy` de `useWalletSendTx`: sondea el nodo hasta ver la delegación (45 s) con un latch que impide reofrecer la compra mientras tanto. Staking/freeze de TRX propio y alquiler de ancho de banda quedan fuera (el proveedor no los vende)
...
- API base URL switches on `__DEV__`; dev IP `10.0.0.239:3000` in `config.ts` may need updating per machine/red
...
- **KYC (Didit) y nudges de verificación**: la verificación corre con el SDK NATIVO embebido (`hooks/useKycVerification.ts`), con fallback a la URL hospedada en el navegador cuando `POST /user/kyc` no devuelve `session_token` — que es lo NORMAL en una sesión REUTILIZADA: el backend solo obtiene el token al crearla. Contrato: `POST /user/kyc[?refresh=1]` → `{ data: url, session_token }` (400 ya verificado, 403 retenido **con `reason: compliance|limit`**, 409 en revisión, 429 lock, 502 proveedor caído; `refresh=1` salta la caché de 60s del backend, salida para quien aterriza en el "esta sesión no existe" del proveedor); `GET /user/kyc?detail=1` → `{ kyc, kyc_status, session_status, on_hold, can_retry }`. **`kyc_status` NO es el estado**: el backend lo pone en `pending` al CREAR la sesión y no lo revierte nunca, así que quien abandonó una verificación sigue en `pending` para siempre — leerlo como "en revisión" dejaba esas pantallas sin un solo botón. Quien decide es `on_hold`/`can_retry`, traducidos a fase por `screens/settings/subpanels/kycPhase.ts` (`verified|review|manual_review|idle`); `session_status` es solo etiqueta (el backend lo calcula por precedencia sobre TODAS las sesiones, puede venir de una vieja). El flag autoritativo de gating sigue siendo `user.kyc` (el backend gatea transfer ≥$500, retiro >$1000, P2P completo, ahorro y seller). Superficies: paso `kyc` del Register wizard (sesión silenciosa, con "Ahora no"; un rechazo ordinario se reintenta ahí mismo, solo un 403 CON `reason` avanza), pantalla `settings/subpanels/KYC.tsx` (re-check por AppState solo en el fallback a navegador + polling 12s mientras se espera) y banner del Home (`hooks/useKycPrompt.ts`: 5 descartes máx, cooldown 5 días, gracia 48h tras abrir sesión de verdad — `markKycSessionStarted()` —, y silencio solo si el caso está RETENIDO — `markKycOnHold()`)
