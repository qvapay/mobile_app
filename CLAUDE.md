...
- Base URL from `/config.ts`: dev `http://10.0.0.208:3000/api`, prod `https://api.qvapay.com`. **Dev fallback (`api/apiHost.ts`)**: la primera petición sondea `/status` de la LAN; si no hay respuesta en 15s (`API_DEV_PROBE_TIMEOUT`), el cliente ENTERO pasa a producción el resto de la sesión (cualquier respuesta HTTP, incluso 5xx, mantiene la LAN). Prod nunca sondea. El interceptor de request fija `baseURL` por petición vía `ensureApiHost()`; SSE y PDF leen `getApiBaseUrl()` en call time — nunca copiar `config.API_BASE_URL` a nivel de módulo.
...
- API base URL switches on `__DEV__`; dev IP `10.0.0.208:3000` in `config.ts` may need updating per machine/red
