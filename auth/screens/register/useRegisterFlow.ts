/**
 * Máquina de estados del alta de cuenta: posición del wizard, formulario,
 * códigos de verificación, la sesión SILENCIOSA que se abre al verificar el
 * email y los handlers de cada paso (registro, PIN de email, teléfono por
 * Telegram, KYC nativo, push y cierre de sesión).
 *
 * Se extrajo de `Register.tsx` (cuerpo de ~410 líneas, de las que ~250 eran
 * esto) para que la pantalla se quede con la composición de los pasos, que ya
 * viven en `register/RegisterSteps.tsx`.
 *
 * Invariante clave preservada: la sesión abierta con el PIN del correo NO
 * flipea `isAuthenticated` — el token va al Keychain y `completeSession()`
 * cierra el flow al final.
 */
...
import type { StepKey } from './RegisterSteps'
...
export const STEPS: readonly StepKey[] = ['name', 'email', 'password', 'emailPin', 'phone', 'phoneCode', 'kyc', 'push']
...
/**
 * Wizard de alta de cuenta: estado, validación derivada y handlers.
 *
 * @param navigation - Navegación del stack (salida a Login y fin del flow).
 * @returns Paso actual, formulario, banderas de UI y las acciones de cada paso.
 */
export default function useRegisterFlow(navigation: NativeStackNavigationProp<RootStackParamList, 'Register'>) {
	...
	return {
		...
		// Reenvío del código de teléfono
