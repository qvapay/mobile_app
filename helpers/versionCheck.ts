...
/**
 * Resultado del chequeo de versión. Cuando `needsUpdate` es false por una
 * razón distinta a "ya estás al día", `reason` la explica y los campos de
 * versión no vienen (es la variante que useAppNavigation castea a su
 * UpdateInfo local `{ needsUpdate, currentVersion?, latestVersion?, storeUrl? }`).
 */
export type UpdateCheckResult = {
	...
	storeUrl?: string
	reason?: 'no-latest' | 'error' | 'cooldown'
}
 ...
 * @returns Same shape as checkForUpdate; `reason: 'cooldown'` when suppressed
 *   by the cooldown.
 */
export const maybePromptUpdate = async (): Promise<UpdateCheckResult> => {
	try {
...
export const markPromptShown = async (): Promise<void> => { try { await AsyncStorage.setItem(LAST_PROMPT_KEY, String(Date.now())) } catch { /* ignore */ } }

/**
 * Opens the store page returned by the version check.
 * @param url - `storeUrl` from maybePromptUpdate.
 */
export const openStore = async (url: string): Promise<void> => { try { if (url) await Linking.openURL(url) } catch { /* ignore */ } }
