...
void main() {
  const email = EmailFormz.dirty('email');
  const password = PasswordFormz.dirty('password');
  ...
  group('LoginState', () {
    ...
    test('returns object with updated messageError when messageError is passed',
        () {
      expect(
        LoginState().copyWith(messageError: messageError),
        LoginState(messageError: messageError),
      );
    });
  });
}
