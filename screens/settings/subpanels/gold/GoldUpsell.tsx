...
import type { GoldPlan } from '../../../../helpers/iap'
...
type GoldBusyFlags = {
	...
	isPurchasingIAP: boolean
	isRestoringPurchases: boolean
}

type GoldUpsellProps = {
	/** Catálogo de planes: la CLAVE alimenta las claves i18n settings.goldCheck.plans.<key>.* */
	plans: Record<GoldPlan, { value: number }>
	selectedPlan: GoldPlan
	onSelectPlan: (plan: GoldPlan) => void
	subscriptions?: ProductSubscription[] | null
	connected?: boolean
	busy: GoldBusyFlags
	isLoading?: boolean
	onSubscribeBalance: () => void
	onSubscribeIAP: () => void
	onRestore: () => void
	insets: EdgeInsets
	theme: Theme
...
const GoldUpsell = ({ plans, selectedPlan, onSelectPlan, subscriptions, connected, busy, isLoading, onSubscribeBalance, onSubscribeIAP, onRestore, insets, theme, textStyles }: GoldUpsellProps) => {
	...
	const storePrice = (() => {
		...
		// `ProductSubscription` es una unión iOS/Android: los campos por plataforma
		// solo existen en su rama, así que se leen por una forma local.
		const sub = subscriptions?.find(s => (s as { productId?: string }).productId === productId) as {
			localizedPrice?: string
			subscriptionOfferDetails?: { offerToken?: string, pricingPhases?: { pricingPhaseList?: { formattedPrice?: string }[] } }[]
		} | undefined
	...
	return (
		<>
			...
			<View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: theme.spacing.xl, gap: theme.spacing.md }}>
				{(Object.entries(plans) as [GoldPlan, { value: number }][]).map(([key, plan]) => (
					<Pressable
						key={key}
						style={[
							{
