...
/** Passkey registrada (`GET /auth/passkey/list`). El nombre evita chocar con
 * el binding `Passkey` del require de react-native-passkey. */
type PasskeyEntry = {
	id: string | number
	name: string
	/** true = sincronizada con el llavero/gestor; false = solo este dispositivo. */
	backed_up?: boolean
...
const Passkeys = () => {
    ...
    const fetchPasskeys = useCallback(async () => {
        ...
        // authApi.getPasskeys declara `ApiResult<unknown[]>`: forma local del listado.
        if (result.success) { setPasskeys(result.data as PasskeyEntry[]) }
        setIsLoading(false)
    }, [])
    ...
    const handleRegisterPasskey = async () => {
        try {
            ...
            // El SDK de passkeys rechaza con formas distintas por plataforma.
            const err = e as { message?: string, code?: string, error?: string } | undefined
    ...
    // Delete a passkey
    const handleDeletePasskey = (pk: PasskeyEntry) => {
        Alert.alert(
