...
/** Estados del registro empresarial (espejo de /settings/company de la web). */
type CompanyStatus = 'pending' | 'reviewing' | 'contacted' | 'approved' | 'rejected'

/** Solicitud de registro empresarial (`GET /user/company`). */
type Company = {
	uuid: string
	company_name: string
...
const STATUS_EXPLAINER: Record<CompanyStatus, string> = {
 ...
 */
const Enterprise = ({ navigation }: NativeStackScreenProps<SettingsStackParamList, 'Enterprise'>) => {
	...
	const { data, isPending, isError, refetch } = useQuery({
		...
		// userApi.getCompanies declara `ApiResult<unknown>`: forma local del envelope.
		queryFn: async () => unwrap(await userApi.getCompanies()) as { companies?: Company[] } | undefined,
		placeholderData: previous => previous,
	})
	const companies = data?.companies ?? []
	const hasApproved = companies.some((company: Company) => company.status === 'approved')
...
type BenefitItemProps = { icon: FontAwesome6SolidIconName, text: string, theme: Theme, textStyles: TextStyles }
...
type DetailRowProps = { label: string, value?: string, theme: Theme, textStyles: TextStyles }

const DetailRow = ({ label, value, theme, textStyles }: DetailRowProps) => (
