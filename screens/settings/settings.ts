...
/** Claves de `verifiedKey` que SettingsMenu.itemStatus sabe resolver (fijado por settings.test.js). */
export type SettingsVerifiedKey = 'phone' | 'telegram' | 'kyc' | 'gold'

/**
 * Fila del catálogo. `screen`/`icon` calzan con el contrato que ya esperan
 * `ui/SettingsSection.tsx` y `ui/particles/SettingsItem.tsx`.
 */
export type SettingsMenuOption = {
    /** CLAVE de i18n (settings.menu.items.*), resuelta con t() en render/búsqueda. */
    title: string
    screen: keyof SettingsStackParamList
    icon: FontAwesome6SolidIconName
    /** Tinte fijo del tile, en hex de 6 dígitos (validado por settings.test.js). */
    color: string
    keywords: string[]
    enabled: boolean
    notifications: number
    verifiedKey?: SettingsVerifiedKey
}

/** Grupo del menú: título (clave i18n) + sus filas. */
...
const settings: Record<SettingsGroupKey, SettingsMenuGroup> = {
    appearance: {
