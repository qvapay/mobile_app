...
const P2PCreate = () => {
    ...
    const workingFields = useMemo(() => {
        if (!selectedCoin || !selectedCoin.working_data) {
            return [];
        }
        try {
            const raw =
                typeof selectedCoin.working_data === 'string'
                    ? JSON.parse(selectedCoin.working_data)
                    : selectedCoin.working_data;
    ...
    const handlePublish = async () => {
        if (type != 'buy' && type != 'sell') {
            Toast.show({
                ...
                text2: 'Debes seleccionar una opción'
            });
            return;
        }
        ...
        if (!selectedCoin) {
            Toast.show({
                type: 'error',
                text1: 'Datos incompletos',
                text2: 'Debes seleccionar una moneda'
            });
            return;
        }
        ...
        if (!amount || !receive) {
            Toast.show({
                type: 'error',
                text1: 'Datos incompletos',
                text2: 'Debes completar los montos de comprar y vender'
            });
            return;
        }
        ...
        if (isNaN(amt) || isNaN(rcv) || amt <= 0 || rcv <= 0) {
            Toast.show({
                type: 'error',
                text1: 'Montos inválidos',
                text2: 'Introduce valores numéricos mayores que 0 para comprar y vender'
            });
            return;
        }
        ...
        if (selectedCoin && workingFields.length > 0) {
            ...
            if (!allFilled) {
                Toast.show({
                    ...
                    text1: 'Faltan datos',
                    text2:
                        'Completa los datos de su cuenta para la moneda seleccionada para comprar y vender'
                });
                return;
            }
        }
        ...
        try {
            ...
            const detailsArray =
                workingFields.length > 0
                    ? workingFields.map(field => ({
                        name: field.name,
                        value: (workingForm[keyFromFieldName(field.name)] ?? '')
                            .toString()
                            .trim()
                    }))
                    : [];
            ...
            if (res.status === 201) {
                Toast.show({
                    type: 'success',
                    text1: 'Listo',
                    text2: 'Tu oferta se ha creado correctamente'
                });
            } else {
                const errMsg = res?.data?.error || 'No se pudo crear la oferta P2P';
                Toast.show({
                    type: 'error',
                    text1: 'Error al crear la oferta',
                    text2: errMsg
                });
            }
        } catch (error) {
                        ...
                        >
                            ...
                            <View style={{ paddingVertical: 2 }}>
                                <Text
                                    style={[
                                        textStyles.h6,
                                        { color: theme.colors.tertiaryText, marginBottom: 2 }
                                    ]}
                                >
                            ...
                            <View style={{ marginTop: 12, marginBottom: 6 }}>
                                ...
                                >
                                    <Text
                                        style={[
                                            textStyles.h5,
                                            { color: theme.colors.secondaryText, marginBottom: 6 }
                                        ]}
                                    >
                                    ...
                                    <Pressable
                                        onPress={() => console.log(selectedCoin.working_data)}
                                    >
                                        <FontAwesome6
                                            name="book"
                                            size={16}
                                ...
                                {workingFields.map(field => {
                                    ...
                                    return (
                                        <QPInput
                                            ...
                                            onChangeText={text =>
                                                setWorkingForm(prev => ({ ...prev, [key]: text }))
                                            }
                            ...
                            >
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    <FontAwesome6
                                        ...
                                        iconStyle="solid"
                                    />
                                    <Text style={[textStyles.h5, { marginLeft: 8 }]}>
                                        Opciones avanzadas
                                    </Text>
                                </View>
                                <FontAwesome6
                                    name={advancedOpen ? 'angle-up' : 'angle-down'}
                                    size={18}
                                    color={theme.colors.primaryText}
                                    iconStyle="solid"
                                />
                                        ...
                                        .map(coin => (
                                            ...
                                            >
                                                ...
                                                <View style={{ marginLeft: 12, flex: 1 }}>
                                                    ...
                                                    >
                                                        Mín: ${adjustNumber(coin.min_out)} | Precio: $
                                                        {adjustNumber(coin.price)}
                                                    </Text>
                                                </View>
