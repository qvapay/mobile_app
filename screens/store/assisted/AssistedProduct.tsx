...
import type { AssistedProductData } from './assistedConstants'

type Props = NativeStackScreenProps<RootStackParamList, 'AssistedProduct'>
 ...
 */
const AssistedProduct = ({ navigation, route }: Props) => {
	...
	// `route.params.product` viaja como `Record<string, unknown>` (types/navigation.ts
	// no modela las formas de esta carpeta): se estrecha al leerlo, sin mutarlo
	const [product, setProduct] = useState<AssistedProductData | null>((route.params?.product as AssistedProductData | undefined) || null)
	const [quantity, setQuantity] = useState(1)
	const [adding, setAdding] = useState(false)
	// `undefined` en la unión porque el re-hydrate vuelca `main_image` tal cual (puede no venir)
	const [selectedImage, setSelectedImage] = useState<string | null | undefined>((route.params?.product as AssistedProductData | undefined)?.main_image || null)
	...
	useEffect(() => {
		...
		const fetchProduct = async () => {
			// shopApi devuelve ApiResult<unknown>: el cast documenta el sobre `{ product }`
			const res = await shopApi.getProduct(uuid) as ApiResult<{ product?: AssistedProductData }>
			if (cancelled) return
			if (res.success && res.data?.product) {
				setProduct(res.data.product)
				setSelectedImage(res.data.product.main_image)
			} else {
				// El `else` cubre también el éxito sin `product`: TS no estrecha a fallo
				toast.error(i18n.t('assisted.common.assistedTitle'), { description: (res as ApiFailure).error })
				navigation.goBack()
			}
		}
		fetchProduct()
	...
	const gallery = useMemo(() => {
		...
		// `filter(Boolean)` no estrecha en TS: el cast recoge lo que el filtro ya garantiza
		return [...new Set([product.main_image, ...extras].filter(Boolean) as string[])].slice(0, 8)
	}, [product])
	...
	return (
		<View style={containerStyles.subContainer}>
			<ScrollView contentContainerStyle={contentPadding} showsVerticalScrollIndicator={false}>
				...
				{/* Quantity + total */}
				<View style={[styles.quantityCard, { backgroundColor: theme.colors.surface }, (theme as Theme & { mode?: string }).mode === 'light' && { borderWidth: 1, borderColor: theme.colors.elevationLight }]}>
