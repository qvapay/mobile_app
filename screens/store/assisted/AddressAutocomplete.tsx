...
/** Sugerencia de Places tal y como la devuelve el proxy del backend. */
type AddressSuggestion = { place_id: string, text: string }

/** Campos estructurados que devuelve `getPlaceDetails` (todos opcionales: Places no siempre los resuelve). */
...
type Props = {
	/** Recibe la dirección estructurada cuando el usuario elige una sugerencia. */
...
const AddressAutocomplete = ({ onSelect }: Props) => {
	...
	useEffect(() => {
		...
		const timer = setTimeout(async () => {
			...
			// shopApi devuelve ApiResult<unknown>: el cast documenta el sobre `{ suggestions }`
			const res = await shopApi.autocompleteAddress({ q, session: ensureSession() }) as ApiResult<{ suggestions?: AddressSuggestion[] }>
	...
	const choose = async (suggestion: AddressSuggestion) => {
		...
		setResults([])
		// shopApi devuelve ApiResult<unknown>: el cast documenta el sobre `{ address, formatted }`
		const res = await shopApi.getPlaceDetails({ place_id: suggestion.place_id, session: ensureSession() }) as ApiResult<{ address?: PlaceAddress, formatted?: string }>
	...
	return (
		<View>
			...
			/>
			{results.length > 0 && (
				<View style={[styles.resultsCard, { backgroundColor: theme.colors.surface }, (theme as Theme & { mode?: string }).mode === 'light' && { borderWidth: 1, borderColor: theme.colors.elevationLight }]}>
