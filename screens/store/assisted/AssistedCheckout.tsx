...
/** Respuesta de `POST /shop/assisted-shopping/checkout` (solo lo que lee la pantalla). */
...
type QuoteSummaryProps = {
	/** Quote server-side; `null` mientras no hay estado de destino elegido. */
	quote: AssistedQuote | null
...
const QuoteSummary = ({ quote, insufficient, balance, total, theme, textStyles }: QuoteSummaryProps) => {

	const { t } = useTranslation()

	return (
	<View style={[styles.summaryCard, { backgroundColor: theme.colors.surface }, (theme as Theme & { mode?: string }).mode === 'light' && { borderWidth: 1, borderColor: theme.colors.elevationLight }]}>
...
const ConfirmPurchaseModal = ({ visible, paying, total, addressSummary, onPay, onClose, theme, textStyles }: ConfirmPurchaseModalProps) => {
 ...
 */
const AssistedCheckout = ({ navigation }: Props) => {
	...
	const [form, setForm] = useState<UsAddressForm>(EMPTY_US_ADDRESS)
	...
	useEffect(() => {
		const load = async () => {
			// shopApi devuelve ApiResult<unknown>: el cast documenta el sobre `{ addresses }`
			const res = await shopApi.getShippingAddresses() as ApiResult<{ addresses?: ShippingAddress[] }>
			if (res.success) {
	...
	useEffect(() => {
		...
		const fetchQuote = async () => {
			// El impuesto lo calcula SIEMPRE el backend: el cliente solo pinta este quote
			const res = await shopApi.getQuote({ state: quoteState }) as ApiResult<{ quote?: AssistedQuote }>
			if (cancelled) return
	...
	const handlePay = async () => {
		...
		// Dos desajustes REALES (pre-existentes) entre módulos ya tipados, no se tocan aquí:
		//  - `buildAddressBody` (ui/store/NewAddressForm) devuelve `phone`/`line2` como
		//    `string | null`, mientras `ShopShippingAddressInput` los declara `string | undefined`.
		//  - `selectedUuid` es `string | null` y `shipping_address_id` espera `string | undefined`
		//    (en la práctica lo cubre el guard de `canPay`).
		// El cast documenta el body tal y como viaja hoy; shopApi devuelve ApiResult<unknown>.
		const res = await shopApi.checkout(body as ShopCheckoutInput) as ApiResult<CheckoutResponse>
		setPaying(false)
		...
		if (res.success && res.data?.ok) {
			...
			// El `else` cubre también el éxito sin `ok`: TS no estrecha a fallo
			toast.error(t('assisted.checkout.toasts.checkoutTitle'), { description: (res as ApiFailure).error })
		}
	}
