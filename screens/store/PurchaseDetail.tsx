...
const getLogoUrl = (logo: string | null | undefined) => mediaUrl(logo) || ''

// Localized labels for receipt fields: i18n KEYS resueltas con t() en render
const receiptLabelKeys: Record<string, string> = {
	...
	return (
		<View style={containerStyles.subContainer}>
			<ScrollView
				...
				refreshControl={createHiddenRefreshControl(isLoading, fetchDetail) as ReactElement<RefreshControlProps>}
			>
				...
				<View style={styles.serviceHeader}>
					...
					<View style={[styles.statusBadge, { backgroundColor: statusColor }]}>
						<Text style={[textStyles.h6, { color: theme.colors.almostBlack, fontWeight: '600' }]}>
							{statusText(purchase.status as string)}
						</Text>
					</View>
				</View>
				...
				<View style={[styles.detailsCard, { backgroundColor: theme.colors.surface }]}>
					...
					<DetailRow label={t('store.purchaseDetail.fields.date')} value={getShortDateTime(purchase.created_at as string)} last />
				</View>
				...
				{purchase.transaction?.uuid && (
					<View style={[containerStyles.bottomButtonContainer, { marginTop: 16 }]}>
						<QPButton
							...
							iconColor="white"
							// El detalle solo trae estos cuatro campos del movimiento: la pantalla
							// Transaction espera el objeto completo, así que se castea aquí
							onPress={() => navigation.navigate(ROUTES.TRANSACTION, { transaction: { uuid: purchase.transaction!.uuid, amount: purchase.amount, status: purchase.status, created_at: purchase.created_at } as unknown as Transaction })}
