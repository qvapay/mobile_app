...
/** Store vive en las tabs de MainStack pero navega a rutas del stack raíz. */
type StoreProps = CompositeScreenProps<
	BottomTabScreenProps<MainTabParamList, 'Store'>,
	NativeStackScreenProps<RootStackParamList>
>

/**
 * Navegación que Store reparte a sus secciones (compuesta tabs + stack raíz).
 * Se exporta porque las tres secciones la reciben como prop.
 */
export type StoreNavigation = StoreProps['navigation']
...
const DepartmentCard = ({ icon, iconStyle = 'solid', color, title, subtitle, compact = false, theme, textStyles, onPress }: DepartmentCardProps) => (
	<Pressable
		style={[
			styles.departmentCard,
			compact && styles.departmentCardCompact,
			{ backgroundColor: theme.colors.surface },
			(theme as ThemeWithMode).mode === 'light' && { borderWidth: 1, borderColor: theme.colors.elevationLight },
		]}
		onPress={onPress}
	>
		<View style={[styles.departmentIcon, { backgroundColor: `${color}1A` }]}>
			{/* `icon` llega como dato desde las tarjetas: FA6 tipa `name` como unión POR estilo */}
			<FontAwesome6 name={icon as FontAwesome6SolidIconName} size={16} color={color} iconStyle={iconStyle as 'solid'} />
		</View>
