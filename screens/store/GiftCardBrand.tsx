...
import type { VoucherPurchaseInput } from '../../api/storeApi'
...
/** Estado del wizard de compra: denominación elegida, importe de rango y paso. */
type PurchaseState = { selectedOffer: StoreOffer | null, rangeAmount: string, step: number }
/** Acciones del wizard: fijar un campo o elegir una denominación. */
type PurchaseAction = { type: 'set', field: string, value: unknown } | { type: 'selectOffer', offer: StoreOffer }

const cleanText = (text: unknown): string => {
	if (!text) return ''
	return String(text).replace(/[•|·]+/g, ' · ').replace(/\s+/g, ' ').trim()
}
const formatSendValue = (send: StoreOffer['send']): string => {
	if (!send || send.value == null) return ''
	return `$${Number(send.value).toFixed(2)} ${send.currency || 'USD'}`
}
const getReceivedValue = (offer: StoreOffer | null | undefined): string => {
...
const OfferRow = ({ offer, selected, onSelect, theme, textStyles }: OfferRowProps) => {
 ...
 */
const GiftCardBrand = ({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'GiftCardBrand'>) => {

	// `country` viaja como `CountryParam` (forma libre): la pantalla lo lee con la
	// forma del catálogo (flag/name), que es lo que mandan todos los call sites
	...
	// Ambos lados son opcionales en tipos; en la práctica siempre llega uno de los
	// dos (la ruta exige brandSlug), y el endpoint de compra exige `brand: string`
	const brand = (detailQuery.data?.brand || brandSlug) as string
	...
	useLayoutEffect(() => {
		const raw = parseFloat((user?.balance || 0) as string)
	...
	const handleConfirm = useCallback(async () => {
		...
		setSubmitting(true)
		// `offer_id`/`country`/`brand` son opcionales en los tipos del catálogo pero
		// el endpoint los exige; el guard de arriba garantiza que hay oferta elegida
		const body: VoucherPurchaseInput = {
			...
			<ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={contentPadding}>
				...
				{step === 1 && (
					<View style={styles.section}>
						...
						<View style={{ gap: 8 }}>
							{offers.map((offer: StoreOffer, idx: number) => (
								<OfferRow
