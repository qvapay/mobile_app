...
import type { StorePurchase } from './storeQueries'
 ...
 */
const MyPurchases = ({ navigation }: NativeStackScreenProps<RootStackParamList, 'MyPurchases'>) => {
	...
	const handlePurchasePress = (purchase: StorePurchase) => { navigation.navigate(ROUTES.PURCHASE_DETAIL, { purchaseId: purchase.id }) }

	// Render purchase item
	const renderItem = ({ item }: ListRenderItemInfo<StorePurchase>) => {
		...
		return (
			<Pressable style={[styles.purchaseItem, { backgroundColor: theme.colors.surface }]} onPress={() => handlePurchasePress(item)} >
				...
				<View style={styles.itemContent}>
					...
					<Text style={[textStyles.caption, { color: theme.colors.secondaryText }]}>{getShortDateTime(item.created_at as string)}</Text>
				</View>
	...
	return (
		<View style={containerStyles.subContainer}>
			<FlatList
				...
				refreshControl={createHiddenRefreshControl(isRefreshing, onRefresh) as ReactElement<RefreshControlProps>}
			/>
		</View>
	)
}
