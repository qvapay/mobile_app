...
/** Estado de filtros del catálogo (país, categoría, búsqueda y página de cliente). */
type GiftCardFilters = { selectedCountry: StoreCountry | null, activeCategory: string, search: string, page: number }
/** Única acción del reducer: fijar un campo. */
type FiltersAction = { type: 'set', field: string, value: unknown }

function filtersReducer(state: GiftCardFilters, action: FiltersAction): GiftCardFilters {
	switch (action.type) {
		case 'set':
 ...
 */
const GiftCards = ({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'GiftCards'>) => {
