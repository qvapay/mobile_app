...
/** Estado de filtros: país seleccionado y texto de búsqueda. */
type TopupFilters = { selectedCountry: StoreCountry | null, search: string }
/** Única acción del reducer: fijar un campo. */
type FiltersAction = { type: 'set', field: string, value: unknown }
...
function filtersReducer(state: TopupFilters, action: FiltersAction): TopupFilters {
	switch (action.type) {
		case 'set':
 ...
 */
const PhoneTopupIndex = ({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'PhoneTopupIndex'>) => {
	...
	const goToBrand = useCallback((brand: StoreBrand) => {
		navigation.navigate(ROUTES.PHONE_TOPUP_BRAND, {
			...
			country: selectedCountry as StoreCountry,
	...
	return (
		<View style={containerStyles.subContainer}>
			...
			>
				...
				<View style={[styles.heroCard, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}>
						...
						<ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingRight: 12, gap: 10 }}>
							{featured.map(f => (
								<Pressable
									...
									style={[styles.featuredItem, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}
								>
