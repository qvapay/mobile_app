...
/** Icono FontAwesome6 + color de marca de una red social (color null = usar un color del theme). */
export type MarketSocialIcon = { icon: string, iconStyle: 'brand' | 'solid', color: string | null }

/** Estado de una orden: clave i18n del label + clave de `theme.colors`. */
export type MarketOrderStatusMeta = { label: string, color: string }
...
export const MARKET_CATEGORY_EMOJIS: Record<string, string> = {
	electronics: '📱',
	fashion: '👕',
	home: '🏠',
	beauty: '💄',
...
export const MARKET_SOCIAL_NETWORKS: Record<string, string> = {
	telegram: 'Telegram',
	whatsapp: 'WhatsApp',
	instagram: 'Instagram',
	facebook: 'Facebook',
 ...
 */
export const MARKET_SOCIAL_ICONS: Record<string, MarketSocialIcon | undefined> = {
 ...
 */
export const MARKET_ORDER_STATUS: Record<string, MarketOrderStatusMeta | undefined> = {
...
export const KIND_LABELS: Record<string, string> = { physical: 'market.kinds.physical', digital: 'market.kinds.digital', service: 'market.kinds.service', giftcard: 'market.kinds.giftcard' }
 ...
 *
 * @param network - Key from MARKET_SOCIAL_NETWORKS.
 * @param value - Stored value (may be a handle like `@shop`).
 * @returns URL for `Linking.openURL`, or null when unlinkable.
 */
export function socialHref(network: string, value: unknown): string | null {
 ...
 *
 * @param min - Minimum effective price.
 * @param max - Maximum effective price.
 * @returns Human price or price range.
 */
export function formatPriceRange(min: number | string | null | undefined, max?: number | string | null): string {
	if (min == null) return ''
 ...
 * @returns Localized country name, or the code as fallback.
 */
// `Intl.DisplayNames` existe en Hermes pero NO en las libs de TS que configura
// el preset RN (es2020.intl queda fuera): forma mínima local del constructor.
type DisplayNamesLike = { of: (code: string) => string | undefined }
type IntlWithDisplayNames = { DisplayNames: new (locales: string[], options: { type: string }) => DisplayNamesLike }
let regionNames: DisplayNamesLike | null = null
let regionNamesLang: string | null = null
export function countryName(iso2: unknown): string {
	const code = String(iso2 || '').toUpperCase()
	if (!/^[A-Z]{2}$/.test(code)) return code
	const lang = i18n.language === 'en' ? 'en' : 'es'
	if (!regionNames || regionNamesLang !== lang) {
		try { regionNames = new (Intl as unknown as IntlWithDisplayNames).DisplayNames([lang], { type: 'region' }) } catch { regionNames = null }
 ...
 *
 * @param shipTo - `{ CC: null | [provincias] }` o null (todos).
 * @returns Summary line, or null when the product ships anywhere.
 */
export function shipToSummary(shipTo: ShipTo | null | undefined): string | null {
