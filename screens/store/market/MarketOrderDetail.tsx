...
type RowProps = { label: string, value: string, theme: Theme, textStyles: TextStyles }

const Row = ({ label, value, theme, textStyles }: RowProps) => (
 ...
 */
const MarketOrderDetail = ({ navigation, route }: NativeStackScreenProps<RootStackParamList, 'MarketOrderDetail'>) => {

	// El param viaja como `Record<string, unknown>` (RootStackParamList): la fila
	// de la lista YA trae el pedido completo, así que se lee con su forma real
	const { order } = (route.params || {}) as { order?: MarketOrder }
	...
	const status = MARKET_ORDER_STATUS[order.status as string] || { label: order.status as string, color: 'placeholder' }
	...
	const copyTracking = () => {
		Clipboard.setString(order.tracking_code as string)
	...
	return (
		<View style={containerStyles.subContainer}>
			<ScrollView contentContainerStyle={contentPadding} showsVerticalScrollIndicator={false}>
				...
				{/* Producto */}
				<View style={[styles.card, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}>
					<View style={styles.productRow}>
						...
						<View style={{ flex: 1 }}>
							...
							<Text style={[textStyles.caption, { color: (theme.colors as Record<string, string>)[status.color] || theme.colors.secondaryText, marginTop: 4, fontWeight: '600' }]}>
								{t(status.label)}
							</Text>
						</View>
					</View>
				...
				<View style={[styles.card, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}>
					...
					<Row label={t('market.orderDetail.date')} value={getShortDateTime(order.created_at as string)} theme={theme} textStyles={textStyles} />
					{!!order.delivered_at && (
						<Row label={t('market.orderDetail.delivered')} value={getShortDateTime(order.delivered_at as string)} theme={theme} textStyles={textStyles} />
					)}
				...
				{!!order.tracking_code && (
					<Pressable
						...
						style={[styles.card, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}
					>
				...
				{!!order.shop && (
					<Pressable
						onPress={() => navigation.navigate(ROUTES.MARKET_STORE, { slug: order.shop!.slug as string })}
						style={[styles.card, styles.shopCard, { backgroundColor: theme.colors.surface }, (theme as ThemeWithMode).mode === 'light' && { borderWidth: 0.5, borderColor: theme.colors.border }]}
					>
