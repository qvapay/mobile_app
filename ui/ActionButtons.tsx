...
// Tipos de navegación: el navigation llega del Home (tab dentro del stack raíz),
// así que resuelve tanto rutas del stack (Add/Withdraw/Send/Savings) como el
// salto al tab hermano P2P
...
const SavingsPill = ({ icon, label, onPress, index, pageProgress, theme }: TileProps) => {
	const style = useAnimatedStyle(() => {
		const p = pageProgress ? pageProgress.value : 0
		return {
			opacity: p,
...
type ActionButtonsProps = {
	navigation: NavigationProp<RootStackParamList & MainTabParamList>
	pageProgress?: SharedValue<number>
}
 ...
 *
 * @param props
 * @param props.navigation - React Navigation object.
 * @param props.pageProgress - SharedValue 0..1 del pager (0 = cuenta, 1 = ahorros).
 */
const ActionButtons = ({ navigation, pageProgress }: ActionButtonsProps) => {
	...
	// El hook aún es JS y su @returns no declara forma — cast local al contrato real
	...
	const savingsActions: ActionItem[] = [
