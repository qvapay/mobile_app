...
const buildRoute = (points: number[][]) => {
	const dists = [0]
 ...
 *
 * @param props
 * @param props.routes - Street-routed polylines this courier alternates between.
 * @param props.initialDelay - Stagger before the first cycle (ms).
 * @param props.msPerPx - Travel pace; cycle duration scales with route length.
 * @param props.scaleX - Rendered-width / MAP_W ratio (y maps 1:1).
 * @param props.accent - Theme accent color.
 */
const Courier = ({ routes, initialDelay, msPerPx, scaleX, accent }: CourierProps) => {
 ...
 *
 * @param props
 * @param props.navigation - React Navigation object used for `navigate()`.
 */
const CashDeliveryCard = ({ navigation }: { navigation: NavigationProp<RootStackParamList> }) => {
	...
	// useKycGate es JS con `@returns {object}` — el retorno colapsa a `{}`;
	// cast local a su forma real hasta que el hook migre a TS
	const { requireKyc, gateVisible, gateMessage, closeGate } = useKycGate() as {
		requireKyc: (opts: { gated?: boolean, message?: string }) => boolean
		gateVisible: boolean
	...
	return (
		<View style={styles.section}>
			...
			{/* OJO: `theme.mode` no existe en el tema (siempre undefined) — bug de
			    runtime pre-existente que se preserva tal cual; el cast de abajo es solo de tipos */}
			<Pressable
				onPress={() => {
					if (!requireKyc({ message: t('ui.cashDelivery.kycGateMessage') })) return
					navigation.navigate(ROUTES.WITHDRAW, { preselectedCoin: 'USDCASH' })
				}}
				style={({ pressed }) => [styles.card, { backgroundColor: theme.colors.surface, opacity: kycVerified ? 1 : 0.55, transform: [{ scale: pressed ? 0.98 : 1 }] }, (theme as Theme & { mode?: string }).mode === 'light' && { borderWidth: 1, borderColor: theme.colors.border }]}>
			...
			{/* `as string`: KycGateModal declara message opcional (undefined) pero el
			    hook guarda null cuando está cerrado — mismo valor que siempre viajó */}
