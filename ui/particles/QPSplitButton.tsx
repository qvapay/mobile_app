 ...
 *
 * @param props
 * @param props.title - Label del botón primario.
 * @param props.onPress - Handler del botón primario.
 * @param props.showBack - Abre el slot del botón Atrás.
 * @param props.onBack - Handler del botón Atrás.
 * @param props.backLabel - Label del botón Atrás (default: el "Atrás" localizado).
 * @param props.check - Desliza el check dentro del primario.
 * @param props.disabled - Deshabilita y atenúa el primario.
 * @param props.loading - Spinner en el primario, bloquea ambos.
 * @param props.backRatio - Fracción de la fila que ocupa el secundario (los modales de filtros usan 0.5 para repartir a partes iguales).
 * @param props.backColor - Fondo del secundario (default `theme.colors.secondary`).
 * @param props.backTextColor - Tinta del secundario (default `theme.colors.buttonText`).
 */
...
const QPSplitButton = ({ title, onPress, showBack = false, onBack, backLabel, check = false, disabled = false, loading = false, backRatio = DEFAULT_BACK_RATIO, backColor, backTextColor }: Props) => {
