...
// aquí el nombre del círculo de peligro llega como prop libre (`icon`), así que
// se relaja el tipo del componente en local (cast de tipos, cero runtime).
...
type AlertDrawerProps = {
	...
	onCancel?: () => void
	icon?: string
	onBeforeExpand?: () => void | Promise<void>
 ...
 *
 * @param props
 * @param props.buttonLabel - Label of the collapsed pill button.
 * @param props.title - Heading of the expanded card.
 * @param props.description - Confirmation copy of the expanded card.
 * @param props.onConfirm - Fired when the (morphed) danger button is pressed while expanded.
 * @param [props.confirmLabel=buttonLabel] - Danger button label while expanded;
 *        crossfades from `buttonLabel` when they differ.
 * @param [props.cancelLabel] - Neutral pill label (defaults to the localized "Cancelar").
 * @param [props.onCancel] - Optional action for the neutral pill (fired after
 *        collapsing). Without it the pill is a plain cancel. The ✕ always just dismisses.
 * @param [props.icon='exclamation'] - FontAwesome6 icon inside the danger circle.
 * @param [props.onBeforeExpand] - Awaited before expanding; use it to
 *        resolve content that is only known at press time (e.g. biometrics state).
 */
const AlertDrawer = ({ buttonLabel, title, description, onConfirm, confirmLabel, cancelLabel, onCancel, icon = 'exclamation', onBeforeExpand, style }: AlertDrawerProps) => {
	...
	return (
		<Animated.View
			...
			{/* The morphing trigger/confirm button — a single persistent element, like the demo.
			    El cast del estilo animado es solo de tipos: QPPressable ES un Animated.View
			    y consume estilos de Reanimated en runtime, pero su prop se declara como
			    StyleProp<ViewStyle> plano. */}
			<QPPressable onPress={handleConfirmPress} style={[styles.confirmButton, { backgroundColor: theme.colors.danger }, confirmAnimatedStyle as StyleProp<ViewStyle>]}>
...
const styles: Record<
	'container' | 'card' | 'cardHeader' | 'iconCircle' | 'closeButton' | 'actionsRow'
	| 'cancelButton' | 'confirmSlot' | 'confirmButton' | 'confirmLabelOverlay',
	ViewStyle
> = {
	container: {
		width: '100%',
