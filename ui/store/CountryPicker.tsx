...
/** Catalog country row from `/api/store/*-catalog?countries` (only the fields this picker reads). */
...
type Props = {
	...
	/** Currently selected country (matched by `code`). */
	value?: CatalogCountry | null
	/** Receives the picked country object. */
	onChange?: (country: CatalogCountry) => void
	/** Trigger text when nothing is selected (default: the localized "Seleccionar país"). */
...
const CountryPicker = ({ countries = [], value = null, onChange = noop, placeholder = null }: Props) => {
	...
	const handleSelect = useCallback((country: CatalogCountry) => {
		onChange(country)
		setOpen(false)
		setQuery('')
	}, [onChange])

	const renderRow = (item: CatalogCountry) => {
		...
		<>
			...
			>
				<Pressable style={styles.overlay} onPress={() => setOpen(false)}>
					...
					>
						<View style={[styles.header, (theme as Theme & { mode?: string }).mode === 'light' && { borderBottomWidth: 0.5, borderBottomColor: theme.colors.border }]}>
