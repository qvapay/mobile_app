...
const formatTimeLabel = (t: number, spanSecs: number) => {
...
const ScrubCursor = ({ state, top, bottom, lineColor, dotColor }: ScrubCursorProps) => {
...
/** Punto activo reportado por `onScrub` durante el scrubbing. */
export type ScrubPoint = { time: number, value: number }

type Props = {
	/** Historial de precios del backend. */
	data: RawPricePoint[]
	/** Color semántico (dot live + cursor). */
	trendColor: string
	/** `({ time, value } | null)` punto activo. */
 ...
 */
const PriceChartPro = ({ data, trendColor, onScrub, height = 220 }: Props) => {
