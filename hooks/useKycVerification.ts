 ...
 */
const useKycVerification = (): { launchKyc: () => Promise<KycLaunchResult>, launching: boolean } => {
	...
	const launchKyc = useCallback(async (): Promise<KycLaunchResult> => {
		setLaunching(true)
		try {
			const resp = await userApi.requestKYCSession()
			if (!resp.success || !resp.data) {
				return { kind: 'request-error', status: resp.status, message: (resp as ApiFailure).error }
			}
