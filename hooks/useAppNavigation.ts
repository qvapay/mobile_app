 ...
 * @param url - e.g. https://qvapay.com/p2p/<uuid> or qvapay://p2p/<uuid>
 * @returns The UUID, or null when the URL is not a P2P link.
 */
const parseP2PUuid = (url: string) => {
 ...
 * @param url - e.g. https://qvapay.com/pay/<uuid> or qvapay://pay/<uuid>
 * @returns The UUID, or null when the URL is not a Pay link.
 */
const parsePayUuid = (url: string) => {
 ...
 * @param url - e.g. https://qvapay.com/store/mi-tienda[/<uuid>]
 * @returns Parsed segments (`{ slug, uuid? }`), or null.
 */
const parseStoreLink = (url: string) => {
 ...
 *
 * @param pendingDeepLinkRef - App-root ref holding a deep link URL captured pre-auth.
 * @returns State AppNavigator needs to render the splash gate, the initial
 *   route and the update modal (`navigation`, `user`, flags de carga,
 *   `updateInfo`/`dismissUpdate`) — tipo exacto por inferencia; `navigation`
 *   llega tipado por la declaración global de types/navigation.ts.
 */
export function useAppNavigation(pendingDeepLinkRef: { current: string | null }) {
	...
	useEffect(() => {
		(async () => {
			const info = (await maybePromptUpdate()) as UpdateInfo | null
	...
	useEffect(() => {
		...
		const onClicked = (event: NotificationClickEvent) => {
