...
- [ ] **Bloqueo de capturas** en frase/backup: pendiente, exige dependencia nativa (FLAG_SECURE en Android; iOS solo detecta/ofusca) → `pod install` y rebuild.
...
**Tanda 1 — TRON (CÓDIGO HECHO 2026-09-13, falta aceptación en device: aún no se ha enviado nada desde la app)**. Decisiones del usuario: solo TRON primero; gate = PIN de Ajustes → Bloqueo + Face ID/Touch ID (sin PIN de bloqueo, el primer envío pide crearlo).
...
**Tanda 2 — EVM (CÓDIGO HECHO 2026-09-14, falta aceptación en device)**: ETH/BNB/POL y USDT/USDC en Ethereum, BNB Chain, Base y Polygon.
...
- Aceptación pendiente: USDT BEP-20 y BNB reales enviados desde la app (necesita BNB para gas) y confirmados en BscScan. Después: BTC.
