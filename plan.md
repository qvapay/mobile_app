...
- **Fase 0** — Tab Invest → **Crypto**: ruta `ROUTES.CRYPTO_SCREEN`, deep link `/crypto`, `screens/crypto/`, `cryptoQueries` (raíz RQ `['crypto']`), i18n `crypto.*`. Rollout: **sin flag desde el 2026-09-20**. El flag `useSelfCustodyFlag` (ajuste local + `features.self_custody`) se borró porque en release era inalcanzable — su interruptor vivía dentro de la sección que él mismo gateaba. La wallet entra por el grupo "Wallet" del catálogo de Ajustes (→ Mi wallet → direcciones, frase, biometría, Nodos, eliminar) y por `WalletHome` en el tab Crypto, para todos los usuarios. Apagarla exige publicar versión.
...
- ~~Backend: `features.self_custody` en `/user/extended`~~ — SIN EFECTO desde el 2026-09-20: el flag se borró y la wallet va encendida para todos.
...
## Swap saldo QvaPay ↔ QUSD (CÓDIGO HECHO 2026-09-16 en los tres repos; TronDealer ACEPTADO en prod el mismo día, faltan qpweb + device)
