...
npx jest wallet/           # 31 tests: router + vectores de derivación
```
...
- `config.ts`: la IP dev del backend (`192.168.0.10:3000`) puede necesitar ajuste por máquina.
...
- [ ] **Publicar `~/webs/rpc-registry`** como repo público `qvapay/rpc-registry` (ya tiene git init + commit) y montar espejo `https://rpc.qvapay.com/registry.json` (Cloudflare Pages). URLs ya cableadas en `wallet/registry/useRegistry.ts`.
